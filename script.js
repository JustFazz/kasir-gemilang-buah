...
function setSubTab(subTab) {
    ...
    if (subTab === 'all') document.getElementById('subtab-all').classList.add('active');
    if (subTab === 'Cash') document.getElementById('subtab-cash').classList.add('active');
    if (subTab === 'Transfer') document.getElementById('subtab-transfer').classList.add('active');
    if (subTab === 'Out') document.getElementById('subtab-out').classList.add('active');
...
function setMode(mode) {
    ...
    if (mode === 'Cash') document.querySelector('.cash-mode').classList.add('active');
    if (mode === 'Transfer') document.querySelector('.transfer-mode').classList.add('active');
    if (mode === 'Out') document.querySelector('.out-mode').classList.add('active');
...
async function renderHistory() {
    ...
    let filtered = allTransactions.filter(item => item.dateOnly === selectedDate);
    ...
    allTransactions.filter(item => item.dateOnly === selectedDate).forEach(item => {
    ...
    document.getElementById('stat-total-transfer').innerText = formatRupiah(totalTransfer);
    ...
    document.getElementById('history-count').innerText = `${filtered.length} Catatan`;
}
...
async function toggleVerify(id) {
    ...
    if (item) {
        ...
        await dbUpdate(item);
        renderHistory();
        showToast(item.verified ? 'Transfer telah diverifikasi!': 'Status verifikasi dibatalkan');
    }
}

// FITUR BARU: EKSPOR DATA KE CSV (DIKELOMPOKKAN SESUAI JENIS TRANSAKSI)
async function exportCSV() {
    ...
    const appendGroup = (title, items) => {
        ...
        if (items.length === 0) {
            ...
            items.forEach(i => {
                const verifiedStr = i.type === 'Transfer' ? (i.verified ? 'Diverifikasi': 'Belum Verifikasi'): '-';
...
// FITUR BARU: IMPOR DATA DARI CSV
function importCSV(e) {
    ...
    reader.onload = async function (event) {
        ...
        for (let line of lines) {
            line = line.trim();
            if (!line || line.startsWith('===') || line.startsWith('LAPORAN') || line.startsWith('ID')) continue;
...
async function handleEditSubmit(e) {
    ...
    if (existing) {
        ...
        existing.note = note || (type === 'Cash' ? 'Pemasukan Cash': type === 'Transfer' ? 'Pemasukan Transfer': 'Pengeluaran');
