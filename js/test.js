import { addToSyncQueue, getSyncQueue, getPendingSyncQueue, updateSyncQueueStatus, syncCreateToFirebase, processSyncQueueItem, processPendingSyncQueue } from "./sync.js";
...
export function initTestGlobals() {
    window.testProcessSyncQueue = async function () {
        try {
            ...
            if (pendingQueue.length === 0) return alert("Tidak ada queue pending.");
    ...
    window.testGetSyncQueue = async function () {
        try {
            ...
            alert(`Jumlah queue: ${queue.length}\n\n${JSON.stringify(queue, null, 2)}`);
        } catch (error) {
    ...
    window.testFirebaseCreateSync = async function () {
        try {
            const queue = await getPendingSyncQueue();
            if (queue.length === 0) return alert("Tidak ada queue pending untuk dites.");

            const item = queue[0];
            if (item.operation !== "create") return alert("Queue pertama bukan operation create.");

            await syncCreateToFirebase(item);
            alert(`BERHASIL DIKIRIM KE FIREBASE!\n\nQueue ID: ${item.queueId}\nTransaction ID: ${item.transactionId}`);
        } catch (error) {
            console.error(error);
    ...
    window.testPendingSyncQueue = async function () {
        try {
            ...
            alert(`Pending queue: ${queue.length}\n\n${JSON.stringify(queue, null, 2)}`);
        } catch (error) {
            alert("GAGAL!\n\n" + error.message);
        }
    };
